...
Start containerized databases and the Ktor server (skip servers if you want to manage them manually):

```bash
./gradlew :server:postgres :server:rethinkdb :server:run
```
...
This starts the server (with databases) and the desktop JVM app in parallel.
